...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const mutation = async <T,>(path: string, body?: unknown, authorization?: string) => { const csrf = await csrfToken(); return api<T>(path, { method: 'POST', body: body ? JSON.stringify(body) : undefined, headers: authorization ? { Authorization: `Bearer ${authorization}` } : undefined }, csrf) }
  const refreshToken = async () => { if (!refreshing.current) refreshing.current = mutation<Login>('/api/v1/auth/refresh').then(result => { setAccessToken(result.accessToken); return result.accessToken }).finally(() => { refreshing.current = null }); return refreshing.current }
  const refresh = async () => { await refreshToken() }
  useEffect(() => { refresh().catch(() => setAccessToken(null)).finally(() => setLoading(false)) }, [])
  const protectedRequest = async <T,>(path: string, init: RequestInit = {}) => {
    if (!accessToken) throw new ApiError({ status: 401, detail: 'Authentication is required.' })
    const request = (token: string) => api<T>(path, { ...init, headers: { ...init.headers, Authorization: `Bearer ${token}` } })
