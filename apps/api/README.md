...
[ADR 0002 — Multi-tenancy và membership](../../docs/adr/0002-multi-tenancy-and-membership.md) chốt business là tenant, membership nhiều business cho một user, role `OWNER`/`ADMIN`/`STAFF` và isolation bằng `tenant_id` lấy từ session/membership đã xác thực. BF-023 thêm migration `V4`; BF-024 thêm `POST /api/v1/businesses`, tạo business và OWNER/ACTIVE membership trong một transaction. Xem [schema business/membership](../../docs/setup/business-membership-schema.md) và [API tạo business](../../docs/setup/business-creation-api.md). Chưa có tenant context runtime, repository nghiệp vụ, authorization hay API chuyển business.
